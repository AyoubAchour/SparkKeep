...
import { listInboxLocal, updateIdeaLocalStatus, deleteIdeaLocal, enqueueOutbox, getOutboxStatusAll, getServerIdByLocalId } from '../src/data/localdb';
...
export default function Inbox() {
  ...
  const [outboxMap, setOutboxMap] = useState<Record<string, { attemptCount: number; lastError: string | null; action: string }>>({});

  async function load() {
    setLoading(true);
    try {
      const rows = await listInboxLocal();
      setItems(rows);
      const pending = await getOutboxStatusAll();
      const map: Record<string, { attemptCount: number; lastError: string | null; action: string }> = {};
      for (const p of pending) map[p.localId] = { attemptCount: p.attemptCount, lastError: p.lastError, action: p.action };
      setOutboxMap(map);
    } catch (e: any) {
  ...
  async function markDone(item: IdeaRecord) {
    ...
    await enqueueOutbox('updateStatus', { localId: item.localId, serverId, status: 'DONE' as IdeaStatus });
    triggerSync();
    load();
  }
  ...
  const filteredItems = items.filter(item => {
    if (filter === 'All') return !item.archived;
    if (filter === 'To Do') return item.status !== 'DONE' && !item.archived;
    if (filter === 'Done') return item.status === 'DONE' && !item.archived;
    return false;
  });

  const allCount = items.filter(i => !i.archived).length;
  const todoCount = items.filter(i => i.status !== 'DONE' && !i.archived).length;
  const doneCount = items.filter(i => i.status === 'DONE' && !i.archived).length;
    ...
    <View style={[styles.container, { backgroundColor: theme.colors.bg }]}>
      <View style={[styles.header, { backgroundColor: theme.colors.cyan, borderColor: theme.colors.black, paddingTop: insets.top + 16 }]}>
        <View style={styles.headerRow}>
          ...
          <Text style={[styles.headerTitle, { color: theme.colors.black }]}>Inbox</Text>
        </View>
      </View>
          ...
          ListEmptyComponent={
            <View style={styles.emptyState}>
              <View style={[styles.emptyIconContainer, { backgroundColor: theme.colors.cyan, borderColor: theme.colors.black }]}>
                <Feather name="inbox" size={48} color={theme.colors.black} />
              </View>
              <Text style={[styles.emptyTitle, { color: theme.colors.black }]}>No Items</Text>
              <Text style={[styles.emptyText, { color: theme.colors.textSecondary }]}>
                {filter === 'All' ? 'Your inbox is empty' : `No ${filter.toLowerCase()} items`}
              </Text>
            </View>
          }
        />
